...
export function CourseAnalysis() {
  ...
  const fetchCourseAnalysis = async () => {
    try {
      ...
      setError('Could not load course analysis data. Please verify the URL or try again later.');
      setLoading(false);
    }
  };
  ...
  if (error || !course) {
    return (
      <div className="max-w-md mx-auto p-8 text-center bg-white rounded-3xl border border-slate-100 shadow-xl mt-12 space-y-4">
        ...
        <Button onClick={() => navigate('/dashboard/courses')} className="w-full bg-primary-dark text-white font-bold py-3 mt-4">
          Back to Course List
        </Button>
      </div>
    );
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <span className="px-3 py-1 bg-emerald-50 text-emerald-700 text-[10px] font-bold rounded-full uppercase tracking-wider">Completed Analysis</span>
            ...
            <h1 className="text-2xl font-bold text-slate-800 font-serif mt-1">{course.course_full_name}</h1>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="shadow-sm lg:col-span-1">
          ...
          <CardContent className="p-0">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/55 border-b border-slate-100 text-[10px] font-bold uppercase tracking-wider text-slate-400">
        ...
        <Card className="shadow-sm lg:col-span-2">
          ...
          <CardContent className="p-6 space-y-6">
            ...
            <div className="flex gap-4 items-center p-4 rounded-2xl border border-dashed border-slate-200 text-xs text-slate-500">
              <BookOpen className="w-5 h-5 text-slate-400 shrink-0" />
              <span>
                To modify grade inputs or update course syllabus info, navigate back to the <strong>Course Management</strong> panel and update the database record.
              </span>
            </div>
          </CardContent>
        </Card>
