...
export default function StudentEventDetailPage() {
  ...
  const formatDate = (dateStr: string) => {
    if (!dateStr) return '—';
    // Handle date-only strings like "2026-07-15" safely (avoid UTC timezone shift)
  ...
  const formatStandaloneTime = (timeStr: string) => {
    if (!timeStr) return '—';
  ...
  const eventImage = event.image_path ? getImageUrl(event.image_path) : (event.image ? getImageUrl(event.image) : null);
        ...
        <View style={{ width: contentWidth, gap: 20, paddingBottom: 60 }}>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.detailsList}>
              {/* Date: prefer start_date, then date */}
              <DetailRow
                icon="calendar-outline"
                label="Date"
                value={formatDate(event.start_date || event.date || '')}
              />
              {/* Start Time: use standalone time field if present, else extract from datetime */}
              {(event.time || (event.start_date && event.start_date.includes('T')) || (event.date && event.date.includes('T'))) ? (
                <DetailRow
                  icon="time-outline"
                  label="Start Time"
                  value={
                    event.time
                      ? formatStandaloneTime(event.time)
                      : formatTime(event.start_date || event.date || '')
                  }
                />
              ) : null}
              {/* End Time: only show when an end_date or end_time exists */}
              {event.end_date ? (
                <DetailRow
                  icon="time-outline"
                  label="End Time"
                  value={event.end_date.includes('T') ? formatTime(event.end_date) : formatDate(event.end_date)}
                />
              ) : event.end_time ? (
                <DetailRow icon="time-outline" label="End Time" value={formatStandaloneTime(event.end_time)} />
              ) : null}
              <DetailRow icon="location-outline" label="Venue" value={event.location || event.venue || '—'} />
              {/* Organizer — API returns organizer_details as nested object */}
                ...
                <DetailRow
                  ...
                  value={
                    [event.organizer_details.first_name, event.organizer_details.last_name]
                      .filter(Boolean)
                      .join(' ') ||
                    event.organizer_details.user?.username ||
                    'Unknown'
                  }
                />
              ) : event.organizer_name ? (
                <DetailRow icon="person-outline" label="Organizer" value={event.organizer_name} />
              ) : null}
              {/* Department from organizer_details */}
              {event.organizer_details?.department ? (
                <DetailRow icon="school-outline" label="Department" value={event.organizer_details.department} />
              ) : null}
          ...
          {regLink ? (
            <View style={styles.card}>
              ...
              <Text style={styles.qrSubtext}>Scan this QR code with your mobile device to register instantly.</Text>
